...
function LearningInterfaceContent() {
  ...
  // 用于强制 ExplainStep 重挂载，从而触发其内部流式生成
  ...
  const initializeLearningSession = async () => {
    try {
      ...
      // 不再在顶层阻塞生成，直接进入界面，交由 ExplainStep 流式生成
      setIsLoading(false);
      return;
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                onClick={() => {
                  // 无阻塞刷新：清空讲解并强制 ExplainStep 重挂载触发流式生成
                  setCurrentStep('EXPLAIN');
                  setAiExplanation('');
                  setLearningContent('');
                  setExplainRefreshKey((k) => k + 1);
                  toast.success('AI讲解内容正在重新生成');
                }}
